...
fn apply_concatenates_plain_and_secret_fragments_in_lexical_order() {
    ...
    fs::create_dir_all(source.join("nvim/init.frag")).unwrap();
    fs::write(source.join("nvim/init.frag/10-base"), b"base config\n").unwrap();
    let secret_envelope = secret::encrypt(b"secret config\n", PASSPHRASE).unwrap();
    fs::write(source.join("nvim/init.frag/20-secret.age"), &secret_envelope).unwrap();
    fs::write(source.join("nvim/init.frag/30-work"), b"work config\n").unwrap();
    ...
    assert!(!target.join("nvim/init.frag").exists());
}
...
fn a_newly_added_fragment_is_picked_up_by_the_next_apply_with_no_registration() {
    ...
    fs::create_dir_all(source.join("shell.frag")).unwrap();
    fs::write(source.join("shell.frag/10-base"), b"alias ls=ls\n").unwrap();
    run_apply(&source, &target);
    ...
    fs::write(source.join("shell.frag/20-extra"), b"alias grep=grep\n").unwrap();
    run_apply(&source, &target);
...
fn diff_shows_drift_between_live_merged_file_and_fresh_render_from_fragments() {
    ...
    fs::create_dir_all(seed.join("shell.frag")).unwrap();
    fs::write(seed.join("shell.frag/10-base"), b"alias ls=ls\n").unwrap();
...
fn save_is_rejected_for_a_composed_target() {
    ...
    assert!(stderr.contains("shell.frag"), "expected message pointing at the fragment dir: {stderr}");

    // Nothing was written into Source's fragments.
    assert_eq!(fs::read_to_string(source.join("shell.frag/10-base")).unwrap(), "alias ls=ls\n");
