...
**Eager package**:
A Package declared in mise's own native `[tools]` table (in `config.toml`, part of Source, written by `add` via `mise config set` — never hand-generated by mysh), installed immediately during Apply via one `mise install`. Exposed on `PATH` through mise's own shim/activation mechanism, not a mysh-generated shim. Diverges from Lazy package in mechanism, not just timing.
