...
/// Writes a real, portable shim file into Source at the mirrored path a package's
...
/// copies it through like any other tracked file. Eager and lazy write the exact same
/// file, differing only by the eager marker line `apply` prewarms from (see
/// ADR-0007) — so `add` needs no `mise` for either, and `--bin` works for both.
fn package_add(source: &Path, specifier: &str, eager: bool, bin_name: Option<&str>) -> Result<String, String> {
    ...
    if dest.exists() {
        return Err(format!(
            "'{bin_name}' is already declared as a package in Source; edit '{}' directly instead",
            dest.strip_prefix(source).unwrap_or(&dest).display()
        ));
    }
    if specifier_already_declared(source, specifier) {
        return Err(format!("'{specifier}' is already declared as a package in Source"));
    }
    ...
    let kind = if eager { "eager" } else { "lazy" };
    Ok(format!("added '{specifier}' ({kind}, as '{bin_name}')\n"))
}

/// Whether any existing shim file in Source already execs `specifier` — a
...
fn specifier_already_declared(source: &Path, specifier: &str) -> bool {
